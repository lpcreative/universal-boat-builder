...
interface SelectedVersionItem {
  groupKey: string;
  versionItemId: string;
  qty: number;
  groupOption: GroupOptionRecord | null;
}
...
function asCandidates(value: SelectionState[string]): string[] {
...
function asQty(value: SelectionState[string]): number {
...
function asBool(value: SelectionState[string]): boolean {
  return value === true;
}
...
function makeLineItemLabel(item: VersionItemRecord): string {
...
function resolveSelectionByGroup(args: {
  ...
  const optionById = new Map<string, GroupOptionRecord>();
  const versionItemsById = new Map(args.bundle.version_items.map((item) => [item.id, item]));
  const selectedRows: SelectedVersionItem[] = [];
  ...
  for (const group of sortedGroups) {
    ...
    const addSelected = (versionItemId: string, qty: number, option: GroupOptionRecord | null): void => {
    ...
    if (groupOptions.length > 0) {
      if (group.selection_mode === "single") {
        const selectedOption = candidates
          ...
          .find((option): option is GroupOptionRecord => Boolean(option));
        if (selectedOption) {
      ...
      if (group.selection_mode === "quantity") {
        ...
        const selectedOption = candidates
          .map((candidate) => optionById.get(candidate) ?? groupOptions.find((option) => option.version_item === candidate))
          .find((option): option is GroupOptionRecord => Boolean(option));
        const fallbackOption = groupOptions[0] ?? null;
        const quantityOption = selectedOption ?? fallbackOption;
...
export function computePricing(
  ...
  const lineItems: PricingLineItem[] = selectedRows
    .filter((row) => {
    ...
    .map((row) => {
      const versionItem = versionItemsById.get(row.versionItemId) as VersionItemRecord;
  ...
  const includedLineItems = [...bundle.version_items]
    ...
    .map((item) => {
      ...
      if (item.msrp == null && item.dealer_price == null) {
        warnings.push(`included version_item "${item.id}" has no msrp/dealer_price; treated as $0 unless configured`);
      }
  ...
  const totals = combined.reduce(
    (acc, item) => {
      ...
      if (typeof item.dealer === "number" && Number.isFinite(item.dealer)) {
